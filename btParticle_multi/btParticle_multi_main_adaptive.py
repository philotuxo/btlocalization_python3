...
class btParticle_multi(threading.Thread):
    ...
    def pf_single_step(self):
        ...
        if self.dataQueue.qsize() > 0:
            ...
            # O-0.0
            self.pf_importance_sampling_diffusion(dongle_mac, beacon_mac, rssi)
            self.pf_mean_point()
            if self.params_pf['size_visuals'] > 0:
                self.send_particles()
    ...
    def pf_selectParticles(self):
        ...
        # for i in range(self.size_d):
        #     self.d_particles[i] = self.d_particles_hat[d_indexes[i]]
        #     p_cumul = getCumulative(self.p_weights[self.size_p * i:self.size_p * (i+1)])
        #     p_indexes = randgen(p_cumul, self.size_p)
        #     # print(range(self.size_p * i, self.size_p * (i+1)))
        #     for j in range(self.size_p):
        #         ind_i = i * self.size_p + j
        #         # print(self.size_p, j, p_indexes)
        #         self.p_particles[ind_i] = self.p_particles_hat[p_indexes[j]]
        ...
        #
        # m = 0
        # u0 = random.uniform(0, 1 / (self.size_p * self.size_d))
        # for i in range(self.size_d):
        #     for j in range(self.size_p):
        #         ind_i = i * self.size_p + j
        #         u = u0 + ind_i /(self.size_d * self.size_p)
        #         # print(u, ' ', end='')
        #         while(p_cumul[m] < u):
        #             m += 1
        #         # print(m)
