...
class btParticle_multi(threading.Thread):
        ...
        for i in range(self.size_d):
            ...
            for j in range(self.size_p):
                ...
                if self.pf_check_limits(self.p_particles_hat[ind_p]):
                    self.p_weights[ind_p] = self.pf_likelihood_mxmd(
    ...
    def pf_likelihood(self, point, sensor, beacon, value):
        ...
        # build the distribution given the grid parameters
        # N(mean, variance, value)
    ...
    def pf_likelihood_mxmd(self, point, sensor, beacon, value):
        # 0-1.3
        # get the closest grid point
        x = round(int((point[0] + self.params_pf["size_grid"]/2)/self.params_pf[
            "size_grid"]) * self.params_pf["size_grid"],1)
...
#        mu = self.data_grid[(x, y)][sensor][beacon][self.params_pf["fp_mu"]]

        mu = self.data_grid[(x, y)][sensor][beacon][0]
        if self.params_pf["fp_sigma"] == "self":
            std = self.data_grid[(x, y)][sensor][beacon][1]
        else:
            std = self.params_pf["fp_sigma"]
        return 1 / (std * normalizer) * np.exp(-0.5 * (value - mu) ** 2 / std**2)
