...
sys.path.append("..")
from lib.btParticlesShow_ui import Ui_btParticleShow
...
ellipse_const = np.sqrt(4.605)

particleQueue = queue.Queue()
SEC_SLEEP = 0.0001
particleColor = QtGui.QColor(180,0,0,0)
groundColor = QtGui.QColor(0,200,0)
meanColor = QtGui.QColor(180,0,0)
varColor = QtGui.QColor(0,0,255,10)
...
class btParticleImageScene(btImageScene):
...
class btParticle_viewer(QtWidgets.QMainWindow):
    def __init__(self, name, configFile, particleQueue = None, logQueue = None):
        self.qt_app = QtWidgets.QApplication(sys.argv)
        QtWidgets.QWidget.__init__(self, None)

        # create the main ui
        self.ui = Ui_btParticleShow()
        self.ui.setupUi(self)
        self.imageScene = btParticleImageScene(self)
        self.pixMap = None
        self.desiredWidth = 640

        self.lQueue = logQueue
        self.parQueue = particleQueue
    ...
    def timerTrigger(self):
        ...
        if self.parQueue.qsize() > 0:
            data = self.parQueue.get()
            ...
            if data[0] == 'P':
                ...
                self.info = data[5]
            if data[0] == 'S':
                size_particles = data[1]
                # prepare particle visuals
                for i in range(size_particles):
                    pointRe = sampleRandomPoint2dUniform(self.params["limits"])
                    point = real2pix(self.params, pointRe)
                    item = btEllipseItem(point, color=particleColor)
                    self.imageScene.addItem(item)
                    self.particle_visuals.append(item)
            ...
            if data[0] == 'P':
                ...
                for particle in self.particles:
                    ...
                    alpha = int(min(self.weights[counter] * 3000,255))
