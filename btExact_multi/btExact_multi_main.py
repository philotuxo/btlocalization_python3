...
class btExact_multi(threading.Thread):
            ...
            for i in range(gauss_mask.shape[0]):
                ...
                for j in range(gauss_mask.shape[1]):
                    ...
                    if temp_y_ind < 0 \
                        ...
                        self.A[ind, ind] = self.A[ind, ind] + gauss_mask[i, j]
                        continue

                    temp_ind = temp_y_ind * self.params["limits_ind"][
                        0] + temp_x_ind

                    self.A[ind, temp_ind] += gauss_mask[i, j]
        self.sparse_A = csr_matrix(self.A.T)
    ...
    def hmm_single_step(self):
        ...
        if self.dataQueue.qsize() > 0:
            ...
            self.hmm_update(dongle_mac, beacon_mac, rssi)
    ...
    def hmm_predict(self):
        ...
        t_s = time.time()
        self.alphaPredict = self.A @ self.alpha
        t_e = time.time()
        print("TIME:                                                                                    %.6f" % (t_e - t_s))
