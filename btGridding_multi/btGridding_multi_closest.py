...
def workerFunction(pQueue, wQueue, dataHist, beacons, dongles, beta = 1.0):
    ...
    while True:
        ...
        if pQueue.qsize() > 0:
            ...
            # buraya hangi veri noktasinin islenecegi geliyor
            # ilgili index ve grid merkezi
            i = data[0]
            j = data[1]
            point = data[2]

            pointList = find_close_points(point, dataHist.keys(), radius = 5,
                                          numMax=1)

            closestPoint = pointList[0]
            print(j, i, closestPoint)
            for dongle in dongles.keys():
                for beacon in beacons.keys():

                    # t = calculate_t(point, pair[0], pair[1])
                    hist = dataHist[closestPoint][dongle][beacon]
